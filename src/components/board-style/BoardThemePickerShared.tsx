...
export interface BoardThemePickerProps {
  ...
  /** Cap the responsive row count (e.g. 3 for the Export Studio). */
  maxRows?: number;
}
...
/**
 * Rows per page. On phones (narrow rail) cap at 3 so the board + preset grid +
 * pager all fit on one screen (e.g. a 6.9" S26 Ultra) without scrolling; on
 * roomier tablet/desktop columns, allow 4.
 */
...
const BoardThemePicker = memo(function BoardThemePicker({
  ...
  // Page to restore when closing the Add/Edit form.
  const returnToPageRef = useRef(0);

  // Responsive columns AND rows from measured width.
  ...
  // Show every built-in AND every saved custom theme — no page cap. A custom
  // theme the user saved must always appear, however many pages that needs,
  // otherwise it looks like the save was lost.
  ...
  // "+" in Presets → remember current page, switch to Custom save form.
  ...
  return (
    <div ref={containerRef} className="flex h-full flex-col gap-4 select-none">
      {/* ── Tabs — hidden while the save form is open (Add/Edit mode) ────── */}
      <div
        role="tablist"
      ...
      {/* ── Pagination (Presets only): ◂ dots ▸, circular ────────────────── */}
