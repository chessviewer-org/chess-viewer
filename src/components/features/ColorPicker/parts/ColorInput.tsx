  ...
  function ColorInput({
    ...
    return (
      <div className="flex items-center gap-2 p-2.5 bg-bg rounded-xl border border-border/50 transition-all shadow-lg">
        <button
          onClick={onToggle}
          className="w-14 h-14 rounded-lg shrink-0 transition-all shadow-lg hover:shadow-xl relative overflow-hidden group outline-none"
          style={{
            background: value
          }}
          aria-label="Open color picker"
        >
          <div className="absolute inset-0 bg-linear-to-br from-white/10 to-black/10 opacity-0 group-hover:opacity-100 transition-opacity" />
        </button>
        ...
        <button
          onClick={onToggle}
          className="p-3 bg-linear-to-br from-accent/20 to-secondary/20 hover:from-accent/30 hover:to-secondary/30 rounded-lg transition-all shadow-lg outline-none"
