 ...
 */
export async function archiveEntries(archivedEntries: ArchivedHistoryEntry[]): Promise<void> {
 ...
 */
export async function deleteArchivedEntry(entryId: number): Promise<ArchivedHistoryEntry[]> {
...
export async function performAutoArchival(
  ...
  const archivedEntries = toArchive.map((e) => convertToArchivedEntry(e, 'auto'));
 ...
 * @returns Promise resolving to the updated archive list
 */
export async function reactivateEntry(entryId: number): Promise<ArchivedHistoryEntry[]> {
  const archive = await loadArchive();
  const updated = archive.filter((e) => e.id !== entryId);
  await saveArchive(updated);
  return updated;
