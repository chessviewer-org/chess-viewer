...
//
// Dynamically generates Open Graph PNG images for shared chess positions.
// Uses Resvg-wasm to convert a manually constructed SVG string to PNG.
//
// Pipeline: FEN FEN -> Board Matrix -> SVG String -> resvg -> PNG Blob -> HTTP 200

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { initWasm, Resvg } from 'https://esm.sh/@resvg/resvg-wasm@3.1.1';

// Pre-load the Resvg WASM module once per cold start.
...
const LICHESS_PIECE_BASE = 'https://lichess1.org/assets/piece/cburnett';

// Cache pieces as base64 data URIs so resvg can resolve them synchronously
// without needing an external HTTP fetcher configured.
const PIECE_NAMES = [
  'wK',
  'wQ',
  'wR',
...
await Promise.all(
  PIECE_NAMES.map(async (name) => {
    try {
      const url = `${LICHESS_PIECE_BASE}/${name}.svg`;
      const res = await fetch(url);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const svgContent = await res.text();
      const base64 = btoa(svgContent);
      PIECE_CACHE[name] = `data:image/svg+xml;base64,${base64}`;
    } catch (e) {
...
serve(async (req) => {
  try {
    ...
    const fen = rawFen.split(' ')[0]; // Extract just the board portion
    const isFlipped = url.searchParams.get('flipped') === 'true';

    // Validate FEN rows
    const rows = fen.split('/');
    if (rows.length !== 8) {
      return new Response('Invalid FEN', { status: 400 });
    }

    const boardSize = 560;
    const squareSize = boardSize / 8;
    const offsetX = (1200 - boardSize) / 2; // Center horizontally
    const offsetY = (630 - boardSize) / 2; // Center vertically

    let svgBoard = '';

    // Draw board background (dark mode matching the app)
    for (let r = 0; r < 8; r++) {
      for (let c = 0; c < 8; c++) {
        const isLight = (r + c) % 2 === 0;
        // Same colors as ChessVision default
    ...
    for (let r = 0; r < 8; r++) {
      ...
      for (let i = 0; i < rowStr.length; i++) {
        ...
        if (/[1-8]/.test(char)) {
          ...
          // Check bounds to prevent overflow from malformed FEN
          if (c < 8) {
            ...
            const cacheKey = `${color}${piece}`;
            const dataUri = PIECE_CACHE[cacheKey];
    ...
    const logoSvg = `<g transform="translate(30, 20) scale(0.7)">
        ...
        <text x="45" y="32" font-family="sans-serif" font-size="28" font-weight="bold" fill="#ffffff">ChessVision</text>
    </g>`;
    ...
    return new Response(pngData, {
      headers: {
        ...
        // Instruct social networks to cache aggressively (1 year)
